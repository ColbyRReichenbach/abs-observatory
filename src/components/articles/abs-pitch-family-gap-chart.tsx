...
export type AbsPitchFamilyGapPoint = {
  family: string;
  label: string;
...
export function AbsPitchFamilyGapChart({ data }: { data: AbsPitchFamilyGapPoint[] }) {
  ...
  return (
    <div className="h-[360px] w-full">
      <ResponsiveContainer width="100%" height="100%" minWidth={0} minHeight={360}>
        <BarChart data={chartData} margin={{ top: 28, right: 34, bottom: 28, left: 4 }} barGap={8}>
          ...
          />
          <Bar name="Catcher" dataKey="catcherPct" fill="#0f766e" radius={[7, 7, 0, 0]} maxBarSize={58} isAnimationActive={false}>
            <LabelList dataKey="catcherChallenges" position="top" style={{ fill: "#746b60", fontSize: 9, fontWeight: 900 }} />
          </Bar>
          <Bar name="Batter" dataKey="batterPct" fill="#2563eb" radius={[7, 7, 0, 0]} maxBarSize={58} isAnimationActive={false}>
