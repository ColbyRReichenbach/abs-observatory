...
export function GameStrip({ games }: GameStripProps) {
    ...
    const canNext = scrollIndex < sortedGames.length - 4;

    const next = () => setScrollIndex(s => Math.min(s + 1, sortedGames.length - 4));
    ...
    return (
        <div className="relative w-full bg-white/50 backdrop-blur-xl border-y border-gray-100 group py-4 overflow-hidden">
            <div className="mx-auto max-w-7xl px-6 flex items-center h-20 relative">
                {/* Navigation Buttons */}
                {canPrev && (
                    <button
                        onClick={prev}
                        className="absolute left-2 z-10 w-8 h-8 rounded-full bg-white border border-gray-200 shadow-lg flex items-center justify-center hover:bg-gray-50 transition-all hover:scale-110"
                    >
                        <ChevronLeft size={16} />
                    </button>
                )}
                {canNext && (
                    <button
                        onClick={next}
                        className="absolute right-2 z-10 w-8 h-8 rounded-full bg-white border border-gray-200 shadow-lg flex items-center justify-center hover:bg-gray-50 transition-all hover:scale-110"
                    >
