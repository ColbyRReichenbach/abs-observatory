...
export default async function TeamsPage({ searchParams }: { searchParams: Promise<{ range?: string }> }) {
  const sp = await searchParams;
  const range = parseRange(sp.range);
  const teams = await getTeamLeaderboard(range);
  return (
    <main className="mx-auto max-w-7xl px-6 pt-32 lg:pt-48 pb-40">
      <div className="flex flex-col items-center text-center mb-20">
        <h1 className="text-6xl md:text-8xl font-display uppercase tracking-[-0.04em] text-gray-900 leading-[1.2] mb-8 py-4 overflow-visible">
          Team <br />
          <span className="opacity-20 italic px-2">Leaderboard</span>
        </h1>
          ...
          <table className="data-table">
            <thead>
              <tr>
                ...
                <th className="text-center">Effectiveness</th>
                <th className="text-right">Avg Rem</th>
              </tr>
            </thead>
            <tbody>
              {teams.length === 0 ? (
