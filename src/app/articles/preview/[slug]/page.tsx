...
function canPreviewDraftArticles() {
  return process.env.NODE_ENV !== "production" || process.env.ENABLE_DRAFT_ARTICLE_PREVIEW === "true";
}

export default async function ArticlePreviewPage({ params }: { params: Promise<{ slug: string }> }) {
  if (!canPreviewDraftArticles()) {
    notFound();
  }
        ...
        <article className="rounded-[2.5rem] border border-black/10 bg-white/80 p-8 shadow-[0_20px_50px_rgba(0,0,0,0.04)] md:p-12">
          <header className="mx-auto max-w-[70ch] border-b border-black/10 pb-8">
            <p className="text-[11px] font-black uppercase tracking-[0.28em] text-[#7d6c54]">
              Draft Preview • {deskMeta.deskName} • {article.status}
            </p>
