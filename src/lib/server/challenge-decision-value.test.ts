...
describe("challenge decision value", () => {
  it("resolves overturn probability with exact, then direction, then global fallback", () => {
    ...
    expect(
      decision.resolveOverturnProbabilityWithFallback({ calledPitch: "called_strike", edgeBucket: "edge" }, rows)?.fallbackTier,
    ).toBe("exact");
    expect(
      decision.resolveOverturnProbabilityWithFallback({ calledPitch: "called_strike", edgeBucket: "near_edge" }, rows)?.fallbackTier,
    ).toBe("direction_only");
    expect(
      decision.resolveOverturnProbabilityWithFallback({ calledPitch: "ball", edgeBucket: "edge" }, rows)?.fallbackTier,
    ).toBe("global");
  });
    ...
    const winRows = [
      {
        ...
        countKey: "3-1",
        sampleSize: 2500,
        battingTeamWinProbability: 0.62,
        confidenceBand: "high",
      },
      {
        fallbackTier: "exact",
        ...
        countKey: "2-2",
        sampleSize: 2600,
        battingTeamWinProbability: 0.56,
        confidenceBand: "high",
      },
    ] as const;
